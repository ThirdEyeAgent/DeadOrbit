...
namespace DeadOrbitCapture.Services
{
    public class DnsServer
    {
        ...
        // Domains to hijack to our proxy
        ...
        private void OnReceive(IAsyncResult ar)
        {
            try
            {
                ...
                var req    = _udp.EndReceive(ar, ref remote);

                // Re-arm listener immediately
                _udp.BeginReceive(OnReceive, null);

                if (req.Length < 12) return;
                HandleQuery(req, remote);
            }
            catch (Exception ex)
            {
                _logger($"[DNS][ERROR] Receive: {ex.Message}");
                try { _udp?.BeginReceive(OnReceive, null); } catch { }
            }
        }

        private void HandleQuery(byte[] req, IPEndPoint remote)
        {
            try
            {
            ...
            {
                _logger($"[DNS][ERROR] HandleQuery: {ex.Message}");
            }
        }

        private void ForwardReal(string hostname,
                                 ushort id,
                                 byte[] req,
                                 IPEndPoint remote)
        {
            ...
            {
                _logger($"[DNS][ERROR] ForwardReal({hostname}): {ex.Message}");
            }
        }

        private void SendResponse(ushort id,
                                  byte[] req,
                                  IPEndPoint remote,
                                  IPAddress[] addrs)
        {
            ...
            {
                _logger($"[DNS][ERROR] SendResponse: {ex.Message}");
            }
        }
